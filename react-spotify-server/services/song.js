...
exports.getSongById = function getSongById(id) {
  return new Promise((resolve) => {
    setTimeout(() => {
      const song = dummySongData.find((song) => song.id == id);
      song ? resolve(song) : resolve(null);
    }, 1000);
  });
};

exports.create = function addSong(songItem) {
  return new Promise((resolve) => {
    setTimeout(() => {
      try {
        songItem.id = new uuidv4();
...
exports.delete = function deleteSong(id) {
  return new Promise((resolve) => {
    setTimeout(() => {
      dummySongData = dummySongData.filter((song) => song.id !== id);
      const superPremitiveCheck = dummySongData.find((song) => song.id === id);
      superPremitiveCheck ? resolve(false) : resolve(true);
    }, 1000);
  });
};
